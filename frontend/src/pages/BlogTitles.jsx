...
const BlogTitles = () => {
 ...
 const [loading, setLoading] = useState(false)
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
   setLoading(true)
   const prompt = `Generate blog tilte for thr keyword  ${input} in the category ${selectedCategory}`
       const { data } = await axios.post(
        '/api/ai/generate-article',
        { prompt, length: selectedCategory.length },
        {
          headers: {
    ...
    <div className='h-full p-6 flex flex-col md:flex-row gap-4 text-slate-700'>
      ...
      >
        <div className='flex items-center gap-3'>
          ...
          <h3 className='text-xl font-semibold'>AI Title Generator</h3>
        </div>
